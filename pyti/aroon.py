...
def aroon_up(data, period):
    ...
    a_up = [((period -
            list(reversed(data[idx-period:idx+1])).index(np.max(data[idx-period:idx+1]))) /
            float(period)) * 100 for idx in range(period, len(data))]
...
def aroon_down(data, period):
    ...
    a_down = [((period -
            list(reversed(data[idx-period:idx+1])).index(np.min(data[idx-period:idx+1]))) /
            float(period)) * 100 for idx in range(period, len(data))]
